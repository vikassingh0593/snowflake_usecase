...
# SHOW commands. On a fresh account nearly all return zero rows, so the signal is
# whether the COMMAND ITSELF is rejected, not the row count. Wrapped the same way.
SHOW_PROBES = [
    ("show.warehouses",              "SHOW WAREHOUSES"),
    ("show.resource_monitors",       "SHOW RESOURCE MONITORS"),
    ("show.integrations",            "SHOW INTEGRATIONS"),
    ("show.external_volumes",        "SHOW EXTERNAL VOLUMES"),
    ("show.dynamic_tables",          "SHOW DYNAMIC TABLES IN ACCOUNT"),
    ("show.iceberg_tables",          "SHOW ICEBERG TABLES IN ACCOUNT"),
    ("show.hybrid_tables",           "SHOW HYBRID TABLES IN ACCOUNT"),
    ("show.streamlits",              "SHOW STREAMLITS IN ACCOUNT"),
    ("show.cortex_search_services",  "SHOW CORTEX SEARCH SERVICES IN ACCOUNT"),
    ("show.semantic_views",          "SHOW SEMANTIC VIEWS IN ACCOUNT"),
    ("show.agents_cowork",           "SHOW AGENTS IN ACCOUNT"),
    ("show.git_repositories",        "SHOW GIT REPOSITORIES IN ACCOUNT"),
    ("show.dbt_projects",            "SHOW DBT PROJECTS IN ACCOUNT"),
    ("show.application_packages",    "SHOW APPLICATION PACKAGES"),
    ("show.managed_accounts_reader", "SHOW MANAGED ACCOUNTS"),
    ("show.shares",                  "SHOW SHARES"),
    ("show.tags",                    "SHOW TAGS IN ACCOUNT"),
    ("show.alerts",                  "SHOW ALERTS IN ACCOUNT"),
    ("show.notebooks",               "SHOW NOTEBOOKS IN ACCOUNT"),
    ("show.ml_classes",              "SHOW CLASSES IN SNOWFLAKE.ML"),
    ("show.cortex_ai_functions",     "SHOW FUNCTIONS LIKE ''AI\\_%'' IN SCHEMA SNOWFLAKE.CORTEX"),
    ("show.snowflake_db_roles",      "SHOW DATABASE ROLES IN DATABASE SNOWFLAKE"),
    ("show.budgets_class",           "SHOW CLASSES IN SNOWFLAKE.CORE"),
    ("show.data_metric_fns_expect_fail", "SHOW DATA METRIC FUNCTIONS IN ACCOUNT"),
]
...
def main() -> None:
    show_body = "".join(BLOCK.format(name=n, sql=q) for n, q in SHOW_PROBES)
    body = "".join(BLOCK.format(name=n, sql=q) for n, q in PROBES)
    with open("sql/p0_probe.sql", "w") as fh:
        fh.write(HEADER.replace("{SHOW_BODY}", show_body) + body + FOOTER)

    flat = [FLAT_HEADER]
    for name, sql in SHOW_PROBES + PROBES:
        flat.append(f"-- [{name}]\n{sql.replace(chr(39) * 2, chr(39))};\n\n")
    with open("sql/p0_probe_flat.sql", "w") as fh:
        fh.write("".join(flat))

    print(f"wrote sql/p0_probe.sql and sql/p0_probe_flat.sql ({len(SHOW_PROBES)} show + {len(PROBES)} function probes)")
