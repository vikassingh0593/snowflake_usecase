...
# snowflake.streaming.validate.compatibility.with.classic. It refuses to start
# unless you either opt into v3-compatible table-name sanitization and column
# identifier normalization, or explicitly disable the check.
#
# Both compatibility flags are set true rather than disabling the validator.
# Turning a check off to silence it answers nothing, and here the v3 behaviour
# is exactly what is wanted: mechanism 1 and mechanism 3 write the same data
# into two tables, and identical naming rules are what keep the comparison
# about latency and credits rather than about incidental schema differences.
#
