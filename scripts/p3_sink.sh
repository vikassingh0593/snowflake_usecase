...
#
# v4 ships a compatibility validator, on by default via
# snowflake.streaming.validate.compatibility.with.classic. It demands three
# things in sequence: v3-style autogenerated table name sanitization, v3-style
# column identifier normalization, and snowflake.validation=client_side.
#
# It is switched OFF here, because none of what it protects applies to this
# setup and the third demand costs something real:
#
#   - Table names are explicit in snowflake.topic2table.map, so autogenerated
#     name sanitization never runs.
#   - Schematization is off, so the columns are RECORD_CONTENT and
#     RECORD_METADATA in both connectors and identifier normalization has
#     nothing to normalize.
#   - client_side validation is a downgrade. v4 validates server_side by
#     default, which checks the config against the real account at startup
#     rather than failing later at runtime - that is how the BouncyCastle
#     problem surfaced during registration instead of during ingestion.
#
# The validator exists for people MIGRATING a v3 connector, so v4 does not
# silently rename their tables. This is a fresh v4 alongside v3, not a
# migration.
#
