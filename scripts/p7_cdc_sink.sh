...
# Debezium's Postgres topic name is <topic.prefix>.<schema>.<table>. The prefix
# is "qc" and the schema is also "qc", so the topics are qc.qc.orders and not
# qc.orders. Subscribing to the short name does not fail -- the broker
# auto-creates the missing topic and the sink sits on it forever reporting
# RUNNING with nothing to read, which is why seven empty topics appeared out of
# nowhere.
TOPICS="qc.qc.dark_stores,qc.qc.customers,qc.qc.products,qc.qc.riders,qc.qc.inventory,qc.qc.orders,qc.qc.order_items"
MAP="qc.qc.dark_stores:CDC_DARK_STORES,qc.qc.customers:CDC_CUSTOMERS,qc.qc.products:CDC_PRODUCTS,qc.qc.riders:CDC_RIDERS,qc.qc.inventory:CDC_INVENTORY,qc.qc.orders:CDC_ORDERS,qc.qc.order_items:CDC_ORDER_ITEMS"

# The seven empty qc.* topics the previous run auto-created. Deleted by `cleanup`
# so they stop looking like real, silent topics.
STRAY="qc.dark_stores qc.customers qc.products qc.riders qc.inventory qc.orders qc.order_items"
...
cleanup)
  echo "== deleting the empty auto-created topics"
  for t in $STRAY; do
    docker exec qc-redpanda rpk topic delete "$t" 2>&1 | sed 's/^/  /'
  done
  echo
  echo "== topics now"
  docker exec qc-redpanda rpk topic list
...
*) echo "usage: $0 diagnose|resnapshot|check|create|status|delete|cleanup"; exit 1 ;;
esac
